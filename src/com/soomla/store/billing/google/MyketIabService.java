 ...
 */
public class MyketIabService implements IIabService {

    public static final String VERSION = "1.0.5";

    public MyketIabService() {
    ...
    public void consumeAsync(IabPurchase purchase, final IabCallbacks.OnConsumeListener consumeListener) {
        mHelper.consumeAsync(purchase, new MyketIabHelper.OnConsumeFinishedListener() {
            @Override
    ...
    private static class OnIabPurchaseFinishedListener implements IabHelper.OnIabPurchaseFinishedListener {
        ...
        public void onIabPurchaseFinished(IabResult result, final IabPurchase purchase) {
            ...
            MyketIabService.getInstance().mWaitingServiceResponse = false;

            if (result.getResponse() == IabResult.BILLING_RESPONSE_RESULT_OK) {
                if (isVerifyPurchasesEnabled()) {
                    MyketIabService.getInstance().verifyPurchases(Arrays.asList(purchase), new VerifyPurchasesFinishedListener() {
                        @Override
                        public void finished() {
                ...
                MyketIabService.getInstance().mSavedOnPurchaseListener.cancelled(purchase);
            } else if (result.getResponse() == IabResult.BILLING_RESPONSE_RESULT_ITEM_ALREADY_OWNED) {

                MyketIabService.getInstance().mSavedOnPurchaseListener.alreadyOwned(purchase);
            } else {

                MyketIabService.getInstance().mSavedOnPurchaseListener.fail(result.getMessage());
            }
        ...
        private void purchaseFinishedSuccessfully(IabPurchase purchase) {
            MyketIabService.getInstance().mSavedOnPurchaseListener.success(purchase);
            purchaseFinished();
        }

        private void purchaseFinished() {
            MyketIabService.getInstance().mSavedOnPurchaseListener = null;
            MyketIabService.getInstance().stopIabHelper(null);
        }
    }
    ...
    public static class IabActivity extends Activity {
        ...
        protected void onCreate(Bundle savedInstanceState) {
            ...
            try {
                ...
                MyketIabService.getInstance().mHelper.launchPurchaseFlow(this, productId, onIabPurchaseFinishedListener, payload);
                MyketIabService.getInstance().mWaitingServiceResponse = true;
                ...
                SoomlaUtils.LogError(TAG, msg);
                MyketIabService.getInstance().mWaitingServiceResponse = false;
                if (MyketIabService.getInstance().mSavedOnPurchaseListener != null) {
                    MyketIabService.getInstance().mSavedOnPurchaseListener.fail(msg);
                    MyketIabService.getInstance().mSavedOnPurchaseListener = null;
                }
            }
        }
        ...
        protected void onActivityResult(int requestCode, int resultCode, Intent data) {
            ...
            if (!MyketIabService.getInstance().mHelper.handleActivityResult(requestCode, resultCode, data)) {
        ...
        protected void onDestroy() {
            ...
            if (!mInProgressDestroy && MyketIabService.getInstance().mWaitingServiceResponse)
            {
...
//                    MyketIabService.getInstance().mSavedOnPurchaseListener.fail(err);
//                    MyketIabService.getInstance().mSavedOnPurchaseListener = null;
//                }
            }
    ...
    public static MyketIabService getInstance() {
        return (MyketIabService) SoomlaStore.getInstance().getInAppBillingService();
    }


    /* Private Members */
    private static final String TAG = "SOOMLA MyketIabService";
    private MyketIabHelper mHelper;
